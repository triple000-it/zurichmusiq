...
async function migrateServiceTable() {
  try {
    ...
    // First, let's check the current Service table structure
    ...
    if (existingServices.length > 0) {
      ...
      existingServices.forEach((service, index) => {
        console.log(`${index + 1}. ${service.title || 'No title'} - ${service.pricing || 'No pricing'}`)
      })
    }

    // Since we can't modify the table structure directly with Prisma,
    // we need to work with the existing structure
    // Let's update the existing services to match our new format
    console.log('🔄 Updating existing services to new format...')

    // Clear existing services and recreate with proper structure
    await prisma.service.deleteMany()

    // Create new services with the correct structure
    const services = [
      {
    ...
    console.log('📝 Creating services with existing table structure...')
